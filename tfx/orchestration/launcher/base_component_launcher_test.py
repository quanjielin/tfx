...
from tfx.orchestration.launcher import test_common
...
class ComponentRunnerTest(tf.test.TestCase):
  ...
  def testRun(self, mock_publisher):
    ...
    component = test_common._FakeComponent(
        name='FakeComponent',
    ...
    self.assertEqual(
        launcher._component_info.component_type, '.'.join([
            test_common._FakeComponent.__module__,
            test_common._FakeComponent.__name__
        ]))
    launcher.launch()
